...
links.Timeline.prototype.getTransformProperty = function(node) {
    var properties = [
        'transform',
        'WebkitTransform',
        'msTransform',
...
links.Timeline.prototype.repaintFrame = function() {
    ...
    if (!dom.content) {
        ...
        this.getTransformProperty(timelines).transform = "translate3d(0px, 0px, 0px)";
    ...
    this.getTransformProperty(dom.content).transform = "translate3d(" + size.contentLeft + "px, 0px, 0px)";
...
links.Timeline.prototype.repaintAxis = function() {
    ...
    if (!axis.frame) {
        ...
        this.getTransformProperty(axis.frame).transform = "translate3d(0px, 0px, 0px)";
...
links.Timeline.prototype.repaintAxisHorizontal = function() {
    ...
    if (hasAxis) {
        ...
        var lineTop = backgroundLine.offsetTop;

        if (axis.backgroundLine) {
            lineTop = size.axis.top;
            //axis.backgroundLine.style.top = size.axis.top + "px";
            axis.backgroundLine.style.height = size.axis.height + "px";
        }

        this.getTransformProperty(backgroundLine).transform = "translate3d(0px, " + lineTop + "px, 0px)";
    ...
    if (hasAxis) {
        ...
        if (axis.line) {
            ...
            this.getTransformProperty(axis.line).transform = "translate3d(" axis.line.offsetLeft + "px, " + size.axis.line + "px, 0px)";
        }
        else {
            // make the axis line
            ...
            this.getTransformProperty(line).transform = "translate3d(0px, " + size.axis.line + "px, 0px)";
            //line.style.left = "0px";
...
links.Timeline.prototype.repaintAxisMinorText = function (x, text) {
    ...
    this.getTransformProperty(label).transform = "translate3d(" + x + "px, " + size.axis.labelMinorTop + "px, 0px)";
...
links.Timeline.prototype.repaintAxisMinorLine = function (x) {
    ...
    this.getTransformProperty(line).transform = "translate3d(" + (x - axis.lineMinorWidth / 2) + "px, " + axis.lineMinorTop + "px, 0px)";
...
links.Timeline.prototype.repaintAxisMajorText = function (x, text) {
    ...
    this.getTransformProperty(label).transform = "translate3d(" + x + "px, " + size.axis.labelMajorTop + "px, 0px)";
...
links.Timeline.prototype.repaintAxisMajorLine = function (x) {
    ...
    this.getTransformProperty(line).transform = "translate3d(" + (x - axis.lineMajorWidth / 2) + "px, " + line.style.top + ", 0px)";
...
links.Timeline.prototype.repaintItems = function() {
    ...
    this.getTransformProperty(frame).transform = "translate3d(0px, " + size.items.top + "px, 0px)";
    ...
    // and redraw terminators
    renderedItems.forEach(function (item) {
        item.updatePosition(timeline);

        if (item.terminator) {
            item.repaintTerminators();
        }
    });
...
links.Timeline.prototype.repaintGroups = function() {
    ...
    this.getTransformProperty(frame).transform = "translate3d(" + size.groupsLeft + "px, " + frame.style.top + ", 0px)";
    ...
    for (var i = current; i < needed; i++) {
        ...
        this.getTransformProperty(labelLine).transform = "translate3d(0px, " + labelLine.style.top + ", 0px)";
        ...
        this.getTransformProperty(itemLine).transform = "translate3d(0px, " + itemLine.style.top + ", 0px)";
    ...
    if (!dom.groups.background) {
        ...
        this.getTransformProperty(background).transform = "translate3d(0px, 0px, 0px)";
    ...
    if (!dom.groups.line) {
        ...
        this.getTransformProperty(line).transform = "translate3d(0px, 0px, 0px)";
        //line.style.left = "0px";
        line.style.width = "100%";
        line.style.height = "0px";
 ...
 */
links.Timeline.prototype.repaintWatermark = function () {
    ...
    if (!options.showWatermark) {
        if (dom.watermark) {
            dom.contentTimelines.removeChild(dom.watermark);
            delete dom.watermark;
        }
    ...
    if (!dom.watermark) {
        if (options.watermark != undefined) {
            ...
            watermark.id = "timeline-watermark";
            watermark.textContent = options.watermark;
            dom.contentTimelines.appendChild(watermark);
            dom.watermark = watermark;
        }
    }

    var watermarkWidth = (size.contentWidth * 0.9);
    dom.watermark.style.width = watermarkWidth + 'px';
    dom.watermark.style.marginLeft = ((size.contentWidth - watermarkWidth) / 2) + 'px';
    var newFontSize = Math.max(size.contentWidth * 0.07, 36);

    var topPadding = ((size.contentHeight - newFontSize) / 2);
    dom.watermark.style.paddingTop = topPadding + 'px';

    dom.watermark.style.fontSize = newFontSize + 'px';
}

/**
 * Redraw the phase dividers
 */
links.Timeline.prototype.repaintPhases = function () {
    var options = this.options,
        dom = this.dom,
        size = this.size;
    ...
    if (!options.showPhases) {
        if (dom.phases) {
            dom.contentTimelines.removeChild(dom.phases);
            delete dom.phases;
        }
    ...
    if (!dom.phases) {
        if (options.phases != undefined) {
            ...
            // create the phase dividers
            var phases = document.createElement("DIV");
            phases.style.position = "absolute";
            phases.style.top = "0px";
            phases.style.height = "100%";

            // add items for every phase
    ...
    if (dom.phases) {
        ...
        for (var i = 0; i < dom.phases.childNodes.length; i++) {
            ...
            this.getTransformProperty(phaseElement).transform = "translate3d(" + startX + "px, 0px, 0px)";
            ...
            this.getTransformProperty(phaseElement.phaseText).transform = "translate3d(" + (endX - startX) + "px, " + (size.contentHeight - endX + startX) + "px, 0px)";
        ...
        if (this.phasesTimer != undefined) {
            clearTimeout(this.phasesTimer);
            delete this.phasesTimer;
        }

        var timeline = this;
        var onTimeout = function () {
            timeline.repaintPhases();
        };
        // the time equal to the width of one pixel, divided by 2 for more smoothness
        var interval = 1 / this.conversion.factor / 2;
        if (interval < 30) interval = 30;
        this.phasesTimer = setTimeout(onTimeout, interval);
    }
}
...
links.Timeline.prototype.repaintCurrentTime = function() {
    ...
    this.getTransformProperty(dom.currentTime).transform = "translate3d(" + x + "px, " + dom.currentTime.style.top + ", 0px)";
...
links.Timeline.prototype.repaintMajorMarker = function () {
    ...
    this.getTransformProperty(dom.majorMarker).transform = "translate3d(" + x + "px, " + dom.majorMarker.style.top + ", 0px)";
...
links.Timeline.prototype.repaintCustomTime = function() {
    ...
    if (!dom.customTime) {
        ...
        this.getTransformProperty(drag).transform = "translate3d(-10px, 0px, 0px)";
        //drag.style.top = "0px";
    ...
    this.getTransformProperty(dom.customTime).transform = "translate3d(" + x + "px, " + dom.customTime.style.top + ", 0px)";
...
links.Timeline.prototype.repaintDeleteButton = function () {
    ...
    if (item && item.rendered && this.isEditable(item)) {
        ...
        this.getTransformProperty(deleteButton).transform = "translate3d(" + right + "px, " + top + "px, 0px)";
...
links.Timeline.prototype.repaintDragAreas = function () {
    ...
    if (item && item.rendered && this.isEditable(item) &&
        ...
        this.getTransformProperty(dragLeft).transform = "translate3d(" + left + "px, " + top + "px, 0px)";
        ...
        this.getTransformProperty(dragRight).transform = "translate3d(" + (right - options.dragAreaWidth) + "px, " + top + "px, 0px)";
...
links.Timeline.prototype.onMouseMove = function (event) {
    ...
    if (params.customTime) {
        ...
        this.getTransformProperty(dom.items.frame).transform = "translate3d(" + frameLeft + "px, " + dom.items.frame.style.top + ", 0px)";
...
/**
 * Repaint the terminators for the item.
 */
links.Timeline.Item.prototype.repaintTerminators = function () {
    // Should be implemented by sub-prototype
};
...
links.Timeline.ItemBox.prototype.createDOM = function () {
    ...
    this.getTransformProperty(divBox).transform = "translate3d(" + this.left + "px, " + this.top + "px, 0px)";
    //divBox.style.left = this.left + "px";
    //divBox.style.top = this.top + "px";

    // add the tooltip
    if (this.tooltip != undefined) {        
        divBox.title = this.tooltip;
    }
...
links.Timeline.ItemBox.prototype.updatePosition = function (timeline) {
    ...
    if (dom) {
        ...
        if (boxAlign == 'right') {
            ...
            domLeft = (left - this.width / 2);
            //dom.style.left = (left - this.width/2) + "px";
        }
        //dom.style.top = this.top + "px";
        this.getTransformProperty(dom).transform = "translate3d(" + domLeft + "px, " + this.top + "px, 0px)";
        ...
        if (axisOnTop) {
            lineTop = axisHeight;
            dotTop = (axisHeight - this.dotHeight / 2);
            ...
            lineTop = (this.top + this.height);
            dotTop = (axisTop - this.dotHeight / 2);
        ...
        this.getTransformProperty(line).transform = "translate3d(" + (left - this.lineWidth / 2) + "px, " + lineTop + "px, 0px)";
        this.getTransformProperty(dot).transform = "translate3d(" + (left - this.dotWidth / 2) + "px, " + dotTop + "px, 0px)";
...
links.Timeline.ItemBox.prototype.setPosition = function (left, right) {
    ...
    this.getTransformProperty(dom).transform = "translate3d(" + (left - this.width / 2) + "px, " + domTop + "px, 0px)";
    this.getTransformProperty(dom.line).transform = "translate3d(" + (left - this.lineWidth / 2) + "px, " + dom.line.offsetTop + "px, 0px)";
    this.getTransformProperty(dom.dot).transform = "translate3d(" + (left - this.dotWidth / 2) + "px, " + dom.dot.offsetTop + "px, 0px)";
};
...
links.Timeline.ItemRange.prototype.updatePosition = function (timeline) {
    ...
    if (dom) {
        ...
        this.getTransformProperty(dom).transform = "translate3d(" + left + "px, " + this.top + "px, 0px)";
        //dom.style.top = this.top + "px";
        //dom.style.left = left + "px";
...
links.Timeline.ItemRange.prototype.setPosition = function (left, right) {
    ...
    this.getTransformProperty(dom).transform = "translate3d(" + left + "px, " + domTop + "px, 0px)";
};
...
/**
 * Redraw the event's terminators, if specified. When an item (ranges only) is selected,
 */
links.Timeline.ItemRange.prototype.repaintTerminators = function () {
    var divBox = this,
        dom = this.dom;

    // create left terminator
    var terminatorLeft = dom.terminatorLeft;
    if (!terminatorLeft) {
    ...
    if (divBox.rendered) {
        ...
        this.getTransformProperty(dom.terminatorLeft).transform = "translate3d(" + terminatorMargin + "px, " + terminatorMargin + "px, 0px)";
        ...
        this.getTransformProperty(dom.terminatorRight).transform = "translate3d(" + (dom.offsetLeft + dom.offsetWidth - terminatorWidth - terminatorMargin) + "px, " + terminatorMargin + "px, 0px)";
...
links.Timeline.ItemDot.prototype.updatePosition = function (timeline) {
    ...
    if (dom) {
        ...
        this.getTransformProperty(dom).transform = "translate3d(" + (left - this.dotWidth / 2) + "px, " + this.top + "px, 0px)";
        //dom.style.top = this.top + "px";
        ...
        this.getTransformProperty(dom.dot).transform = "translate3d(" + dom.dot.offsetLeft + "px, " + ((this.height - this.dotHeight) / 2) + "px, 0px)";
...
links.Timeline.ItemDot.prototype.setPosition = function (left, right) {
    ...
    this.getTransformProperty(dom).transform = "translate3d(" + (left - this.dotWidth / 2) + "px, " + domTop + "px, 0px)";
};
...
links.Timeline.prototype.getItem = function (index) {
    ...
    if (item.tooltip) {
        properties.tooltip = item.tooltip;
    }

    if (item.terminator) {
        properties.terminator = item.terminator;
    }
...
links.Timeline.prototype.createItem = function(itemData) {
    ...
    var data = {
        ...
        terminator: itemData.terminator
    };
...
links.Timeline.prototype.changeItem = function (index, itemData, preventRender) {
    ...
    var newItem = this.createItem({
        ...
        'tooltip': itemData.hasOwnProperty('tooltip') ? itemData.tooltip : oldItem.tooltip,
        'terminator': itemData.hasOwnProperty('terminator') ? itemData.terminator : oldItem.terminator
    });
...
function setTranslad3d(element, x, y, z) {
